...
This section will present the maps which resulted from the M2 NBRA-predictions on both Instagram and Flickr media objects from the Canton of Zug. The arrangement of maps in figure \ref{fig:map_cluster_2} shows the NBRAs spatial distribution in parallel comparison between the two SMPs. The amount of M2 predictions per classification and SMP are listed in table \ref{tab:amount_class_NBRAs} as well as in the maps themselves as \textit{feature count}.
